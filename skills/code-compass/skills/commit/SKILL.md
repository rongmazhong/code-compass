...
- **VAPD_ID**（可选，自动携带）：取自 `.harness/state/workflow-state.json` 的 `vapd_id` 字段
  - 需求 `VR` 开头 / 缺陷 `VB` 开头 / 任务 `VT` 开头，如 `VR12345`、`VB2024`、`VT7788`
   - 由 product-analysis 阶段用 `bash scripts/vapd.sh <ID>` 记录；未记录时该部分省略
- **描述**（必填）：简洁说明本次改动
...
2. 确定 `type` 与描述。
3. 运行 **`bash scripts/commit.sh <type> <描述...>`**，脚本会自动拼接 `vapd_id` 并 `git commit`。

   ```bash
   bash scripts/commit.sh feat 开发登录接口
...
从源头杜绝"跳过分析就开发"。提示先加载 `product-analysis` 子 skill。

- 豁免：`bash scripts/commit.sh --exempt <type> <描述>` 跳过阶段校验（如一次性脚手架、hotfix），
  需在回复中说明豁免理由。
- 关闭全部闸门（不推荐，仅调试）：环境变量 `CODE_COMPASS_GUARD=off`。

## 记录 VAPD 标识

若用户在需求描述中显式给定 VAPD ID，先运行 `bash scripts/vapd.sh <ID>` 写入 state，
后续所有 `bash scripts/commit.sh` 都会自动携带。

```bash
bash scripts/vapd.sh VR12345   # 记录需求标识
bash scripts/vapd.sh           # 查看当前已记录的标识
```
