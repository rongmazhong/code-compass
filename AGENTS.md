...
> 未经分析直接落地的改动视为偏离方法论，会被 `guard` / `commit` 子 skill 拦截。
...
否则视为「偏离方法论」，应先加载 `product-analysis` 子 skill 收敛需求、生成 spec，**不得直接编辑实现**。
...
- 动手前先加载 `guard` 子 skill（或 `bash scripts/status.sh --guard`）做闸门校验。
...
| 我要动手 / 是否可编码 / 校验阶段 | `guard`（`bash scripts/status.sh --guard`） | 校验阶段，偏离则拦截 |
...
1. 加载 `guard` 子 skill（或调 `bash scripts/guard.sh`）校验当前阶段；
2. 若校验未通过：先产出 spec 骨架（加载 `product-analysis` 子 skill）或一份澄清清单，再继续；
...
若 agent 准备写代码而 `state` 仍处 `idea` / `product-analysis`，`guard` 子 skill
会输出**黄色提醒**并以非 0 退出（视为偏离）。agent 应停下，先走 `product-analysis` 子 skill，
或显式使用豁免（见下）并说明原因。
...
- `bash scripts/dev.sh --force`：强制进入开发（已知 spec 在脑中/极小改动）。
- `bash scripts/commit.sh --exempt <type> <描述>`：跳过提交期阶段校验（如一次性脚手架、hotfix）。
- 环境变量 `CODE_COMPASS_GUARD=off` 关闭全部闸门（不推荐，仅调试用）。

## 意图 → 子 skill

agent 按用户意图直接加载对应子 skill（需要机械操作时由子 skill 散文指示调 `scripts/*.sh`）：

| 用户意图 | 子 skill | 机械操作 |
|----------|----------|----------|
| 启用库 / 接入项目 | `using-code-compass` | `bash scripts/use.sh` |
| 初始化 `.harness/` 运行基座 | `init` | `bash scripts/init-harness.sh` |
| 需求分析 / 出方案（**默认第一步**） | `product-analysis` | `bash scripts/product-analysis.sh` |
| 按 spec 开发实现 | `dev` | 开头 `bash scripts/guard.sh` + `bash scripts/worktree.sh` |
| 闸门校验 | `guard` | `bash scripts/guard.sh` |
...
| 查看状态 / 激活续跑 | `status` | `bash scripts/status.sh` |
...
`bash scripts/commit.sh <type> <描述...>` 提交，避免手写 `git commit -m` 漏带标识。
...
- `docs/overview.md` —— 项目概览（做什么、技术栈、快速开始、**下一步应加载 `product-analysis` 子 skill**）
- `docs/architecture.md` —— 架构设计（分层、数据流、关键决策）
- `docs/modules.md` —— 核心模块（目录职责与边界）
- `docs/api.md` —— 功能清单及 API 接口文档

更新文档加载 `wiki` 子 skill（调 `bash scripts/wiki.sh` 重建索引）或 `bash scripts/wiki.sh <overview|architecture|modules|api>`（重建指定文档）。
# <<< code-compass <<<
