...
code-compass 通过 skills CLI 一键安装：
...
> 💡 `npx skills add` 会从技能注册表拉取并注册 skill。它**仅安装 `SKILL.md`**，
> 因此首次使用还需执行一次下方引导，把完整工具（CLI、模板、子 skill）拉到本地。

### 首次使用（仅需一次，拉取完整工具）

```bash
CC_HOME="$HOME/.local/share/code-compass"
...
code-compass use-code-compass      # 注册并启用全部子 skill
```
