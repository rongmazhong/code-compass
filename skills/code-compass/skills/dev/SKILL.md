...
`idea` / `product-analysis`（尚未生成已确认 spec），**直接拦截**并提示先运行
`code-compass product-analysis <slug>`。这是把"先分析后开发"硬化为硬约束的核心拦截点。

- 豁免：`code-compass dev --force <slug>` 强制进入（并会在回复中说明豁免理由）；
...
- **统一格式**：`<type>: #{VAPD_ID}#<描述>`
  - `type`：`feat` / `fix` / `docs` / `refactor` / `test` / `chore` / `style` / `perf` / `build` / `ci`
  - `VAPD_ID`：取自 `workflow-state.json` 的 `vapd_id`（即 product-analysis 阶段用
    `code-compass vapd <ID>` 记录的 VR/VB/VT 标识）；若未记录则省略 `#VAPD_ID#` 部分，退化为 `<type>: <描述>`
  - 示例：`feat: #VR12345#开发登录接口`
- 始终用 **`code-compass commit <type> <描述...>`** 提交（它会自动拼接 vapd_id），
  不要手写 `git commit -m`。在 worktree 内执行时，提交落在 `feat/<slug>` 分支。
