...
name: vapd
description: >
    记录/查看 VAPD 标识（VR 需求 / VB 缺陷 / VT 任务），写入 workflow-state.json，供提交携带。用户说「记录 VAPD / 设置需求 ID / 这个缺陷编号是 VB2024」或 product-analysis 阶段需落 ID 时加载。
---
...
- 用户加载 `vapd` 子 skill（agent 调 `bash scripts/vapd.sh [ID]`）
- 用户说"记录 VAPD"、"设置需求 ID"、"这个缺陷编号是 VB2024"
- `product-analysis` 阶段需要把用户显式给定的 ID 落到状态机

## 用法

### `bash scripts/vapd.sh`（无参）

- 若已 init，打印当前活跃变更的 `vapd_id`（未设置则提示）。
- 未 init 时提醒先加载 `init` 子 skill。

### `bash scripts/vapd.sh <ID>`
