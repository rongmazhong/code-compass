...
| `code-compass upgrade [--self]` | 刷新已 init 项目的 harness 配置（config.json + workflow-state.json）；`--self` 从 `upgrade_source` 拉取并合并 skill 库 |
| `code-compass qa` | 运行测试/静态检查（读 `rules/workflow.md` 的「测试」「静态检查」），全绿则推进活跃变更至 `verified` |
| `code-compass verify` | 逐条核对 spec Requirement 是否达成、勾选 tasks，全部通过则推进至 `reviewed` |
| `code-compass review` | 生成审查包（变更统计 + spec Requirements 对照 + 审查清单），供跨模型二审 |
