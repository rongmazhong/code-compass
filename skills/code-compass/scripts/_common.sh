...
> 由 \`bash scripts/init-harness.sh\` 生成，结合 code-compass 方法论。
...
1. **product-analysis**：加载 \`product-analysis\` 子 skill（运行 \`bash scripts/product-analysis.sh <name>\`），柏拉图式发问确定需求范围，
     产出 \`.harness/openspec/changes/<slug>/{proposal.md,tasks.md,specs/}\`。
2. **dev**：加载 \`dev\` 子 skill（运行 \`bash scripts/dev.sh <name>\`），按 spec 进行
...
> 由 `bash scripts/init-harness.sh` 生成。本文件把「方法论优先」从软建议硬化为 agent 必须遵循的约束。
...
- 动手前先加载 `guard` 子 skill（或调 `bash scripts/guard.sh` / `bash scripts/status.sh --guard`）做闸门校验。
...
准备写代码而 `state` 仍处 `idea` / `product-analysis`，`guard` 子 skill 会输出黄色提醒
...
> 由 `bash scripts/init-harness.sh` 引导生成。
...
代码改动默认先走方法论：加载 \`product-analysis\` 子 skill（运行 \`bash scripts/product-analysis.sh <name>\`）收敛需求、生成 spec，
再加载 \`dev\` 子 skill（运行 \`bash scripts/dev.sh <name>\`）实现。直接用 \`bash scripts/guard.sh\` 校验阶段是否可动手。
