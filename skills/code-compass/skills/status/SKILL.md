...
name: status
description: >
   查看工作流阶段进度、激活续跑，或做阶段闸门校验（guard）。运行 `code-compass status` / `status activate` / `guard` 或说"现在到哪 / 能否动手"时触发。
---
...
`code-compass guard` 的独立方法论见 `skills/guard/SKILL.md`（闸门逻辑、豁免机制同该 skill）。
